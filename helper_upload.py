...
def upload_theme(name, driver):
    driver.get("https://vm1.wproto.net/modules/wp-toolkit/index.php/index/themes")
    time.sleep(2)
    upload_plugin = driver.find_element_by_xpath("//button[@data-test-id='toolbar-upload-button']")
    upload_plugin.click()
    driver.find_element_by_xpath("//input[@type='file']").send_keys(os.getcwd()+"/themes/{}".format(name))
    time.sleep(2)
    ok_button = driver.find_element_by_xpath("//button[@type='button' and @class='pul-button pul-button--lg pul-button--primary pul-button--active']")
    ok_button.click()
    while(True):
        ...
        try:
            driver.find_element_by_xpath("//span[@class='pul-form-field__error']")
            shutil.move("themes/{}".format(name), "rejected_themes/{}".format(name))
            break
        except:
            pass
